...
&lt;/div&gt;
	...
	<longdesc>
		<p>Occasionally, you may want to visually group a set of buttons to form a single block that looks contained like a navigation component. To get this effect, wrap a set of buttons in a container with the <code>data-role="controlgroup"</code> attribute ¡ª the framework will create a vertical button group, remove all margins and drop shadows between the buttons, and only round the first and last buttons of the set to create the effect that they are grouped together. </p>
...
&lt;h1 id="orientation"&gt;orientationchange Not Supported on this Device.&lt;/h1&gt;
	...
	<longdesc>
		...
		<p>The goal of this model is to allow developers to create websites using best practices ¡ª where ordinary links will "just work" without any special configuration ¡ª while creating a rich, native-like experience that can't be achieved with standard HTTP requests.</p>
...
&lt;/div&gt;
	...
	<longdesc>
		<p>Triggered when a horizontal drag of 30px or more (and less than 30px vertically) occurs within 1 second duration but these can be configured:
			<ul>
				<li><code>$.event.special.swipe.scrollSupressionThreshold</code> (default: 10px) ¨C More than this horizontal displacement, and we will suppress scrolling.</li>
				<li><code>$.event.special.swipe.durationThreshold</code> (default: 1000ms) ¨C More time than this, and it isn't a swipe.</li>
				<li><code>$.event.special.swipe.horizontalDistanceThreshold</code> (default: 30px) ¨C Swipe horizontal displacement must be more than this.</li>
				<li><code>$.event.special.swipe.verticalDistanceThreshold</code> (default: 30px) ¨C Swipe vertical displacement must be less than this.</li>
			</ul>
		</p>
...
&lt;/div&gt;
		...
		<ul>
			<li>Form elements elsewhere on the page - select menus in particular - can fail to respond to user interaction when an <em>empty</em> absolute positioned element is placed within a fixed position element. In rare cases¡ªand specific to Android 2.2¡ªthis can cause <em>entire pages</em> to fail to respond to user interaction. This can seemingly be solved by adding any character to the absolute positioned element, including a non-breaking space, and in some cases even whitespace.</li>
...
$( document ).on( "vclick", "p", function() {
	$( this ).append( "&lt;span style='color:#108040;'&gt; vclick fired... &lt;/span&gt;" );
		...
		<dd><p>The virtual mouse events can also be configured:</p>
			<ul>
				<li><code>$.vmouse.moveDistanceThreshold</code> (default: 10px) ¨C More than this, then it is a scroll event. The vmousecancel event is called and the TouchMove event is cancelled.</li>
				<li><code>$.vmouse.clickDistanceThreshold</code> (default: 10px) ¨C If a vclick event was already captured and is in the block list, then vclicks less than this distance are ignored.</li>
				<li><code>$.vmouse.resetTimerDuration</code> (default: 1500ms) ¨C More time than this, then it is not a touch event. Scroll, TouchMove and TouchEnd events use this. The block list is cleared.</li>
			</ul>
		</dd>
	</longdesc>
	<added>1.0</added>
